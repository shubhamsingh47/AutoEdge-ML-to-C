...
from src.utils import generate_unique_header_name
from src.converter.linear import LinearConverter
from src.utils import ensure_dir, detect_linear_model_kind
...
def convert_model(
    ...
    """
    Convert any sklearn linear model .pkl → .h header file.
    """
    logger.info("Starting model conversion: %s", model_path)
    ...
    try:
        ...
        if validate:
            ...
            validate_linear_model_exported(converter.model)
        ...
        raise CustomException("Model conversion failed",sys)
...
def cli_entry(argv: Optional[list] = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="model2c",
        description="Convert sklearn linear .pkl into C header (.h)"
    )
